...
const (
	...
	//DefaultBitLength to transfer
	DefaultBitLength = 24

	defaultTransmissionChanLen = 64
)
...
type CodeTransmitter interface {
	Transmit(uint64, int, uint, int) error
	Wait()
	Close() error
}
...
func (t *NativeTransmitter) Transmit(code uint64, protocol int, pulseLength uint, bitLen int) error {
...
func (t *NativeTransmitter) transmit(trans transmission) {
	for retry := 0; retry < TransmitRetries; retry++ {
		for j := int(trans.bitLen) - 1; j >= 0; j-- {
...
func (t *NullTransmitter) Transmit(code uint64, protocol int, pulseLength uint, bitLen int) error {
	if protocol < 1 || protocol > len(Protocols) {
		return fmt.Errorf("Protocol %d does not exist", protocol)
	}
